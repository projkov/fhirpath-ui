...
export function useFHIRPathUI() {
    ...
    const authorizationHeader = getFromLocalStorage<Array<SettingItem>>(
        'settings',
    )?.find((settingItem) => settingItem.id === 'authorization_header')?.id;
    ...
    const handleShareResult = () => {
        copyToClipboard(
            result.map((resItem) => JSON.stringify(resItem)).join(', '),

...
function sortHistoryItem(
  items?: Array<RequestsHistoryItem | ExpressionsHistoryItem>
) {
  return items
    ? items.sort(
        (a, b) =>
...
export function getRequestsHistory() {
  return sortHistoryItem(
    getFromLocalStorage<Array<RequestsHistoryItem>>(StorageKey.Requests)
  );
}

export function getExpressionsHistory() {
  return sortHistoryItem(
    getFromLocalStorage<Array<ExpressionsHistoryItem>>(StorageKey.Expressions)
  );
}

function updateHistory<T>(key: string, newItem: T) {
  const appSettings = getFromLocalStorage<Array<SettingItem>>(
    StorageKey.Settings
  );
  const settingsKey =
    key === StorageKey.Requests
      ? SettingsKey.REQ_HIST_MAX
      : SettingsKey.EXPR_HIST_MAX;
  const maxNumberOfHistoryItems =
    appSettings?.find((settingItem) => settingItem.id === settingsKey)?.value ??
    100;
